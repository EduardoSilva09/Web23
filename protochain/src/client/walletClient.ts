...
function sendTx() {
  ...
  rl.question('To Wallet:', (toWallet) => {
    ...
    rl.question('Ammount: ', async (amountStr) => {
      ...
      // Saída de transação de troco
      const remainingBallance = (balance - amount - fee);
      txOutputs.push(
        new TransactionOutput({
          toAddress: myWalletPub,
          amount: remainingBallance
        } as TransactionOutput)
      )

      const tx = new Transaction(
        { txInputs, txOutputs } as Transaction
      );

      tx.hash = tx.getHash();
      tx.txOutputs.forEach((txo, index, arr) => {
        arr[index].tx = tx.hash;
      })

      console.log(tx);
      console.log("Remaining Balance: " + remainingBallance);
